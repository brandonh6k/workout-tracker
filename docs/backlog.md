...
- [ ] **Separate template vs scheduled workout concepts**: Current model conflates "template" (exercise list with sets/reps) and "scheduled workout" (template + specific weights for a day). Need to rethink:
...
- [ ] **Simplify rep target in templates**: Change from rep range (min-max) to single rep target. Ranges add complexity without value - you know what you're aiming for.
